...
(define-record-type il:checkpoint
 (fields continuation expression environmet count))
...
;;;\needswork: To walk il:continuation and il:checkpoint.
...
(define (il:walk1 f x)
 (cond ((eq? x #t) #t)
       ...
       ((and (il:closure? x))
	(il:make-closure (il:closure-expression x)
			 (map (lambda (x)
			       (make-il:binding
...
(define (il:walk2 f x x-prime)
 (cond
  ...
  ((and (il:closure? x) (il:closure? x-prime)
	(eq? (il:closure-expression x)
	     (il:closure-expression x-prime))
	(= (length (il:closure-environment x))
	   (length (il:closure-environment x-prime)))
	(every (lambda (x x-prime)
		(eq? (il:binding-variable x) (il:binding-variable x-prime)))
	       (il:closure-environment x)
	       (il:closure-environment x-prime)))
   (il:make-closure (il:closure-expression x)
		    (map (lambda (x x-prime)
			  (make-il:binding
			   (il:binding-variable x)
			   (il:walk2 f
				     (il:binding-value x)
				     (il:binding-value x-prime))))
			 (il:closure-environment x)
			 (il:closure-environment x-prime))))
...
(define (il:walk1! f x)
 (cond ((eq? x #t) #f)
       ...
       ((and (il:closure? x))
	(for-each (lambda (x) (il:walk1! f (il:binding-value x)))
		  (il:closure-environment x)))
       (else (error #f "Can't walk1!"))))
