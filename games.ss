...
(library
 ...
 (export make-state state-board state-cache-for-x state-cache-for-o state-player
	 ...
	 draw? legal-moves random-game valid? *tic-tac-toe* *hexapawn*)
 (import (rnrs)
	 (QobiScheme)
...
;;;             have to say to empty anymore.
 ...
 (define-syntax one-value
  (syntax-rules ()
   ((one-value e)
    (let ((values (domain e)))
     (if (null? values) (fail) (first values))))))
 ...
 (define (legal-moves rules state)
  (remove-duplicatesp
 ...
 (define (random-game sentences)
  (let ((rules (sentences->rules sentences)))
   (let loop ((states (list (initial-state rules))))
    (if (or (win? rules (first states)) (draw? rules (first states)))
	(reverse states)
	;; needs work: to make random
	(loop (cons (first (legal-moves rules (first states))) states))))))

 (define (valid? rules games)
  (every (lambda (game)
	  (and (initial-state? rules (first game))
	       (or (win? rules (last game)) (draw? rules (last game)))
	       (every (lambda (state next-state)
		       (member next-state (legal-moves rules state)))
		      (but-last game)
		      (rest game))))
	 games))
