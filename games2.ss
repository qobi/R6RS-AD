...
(library
 ...
 (export make-state state-board state-cache-for-x state-cache-for-o state-player
	 *lexicon* remove-duplicates sentence->rule sentences->rules
	 initial-state? win? draw? *tic-tac-toe* *hexapawn*)
 (import (rnrs) (QobiScheme) (stochastic-scheme) (stochastic-promises))
...
;;;             have to say to empty anymore.
 ...
 (define (a-member-of d) (draw (map (lambda (x) (cons x (/ (length d)))) d)))

 (define-syntax amb (syntax-rules () ((amb e1 e2) (if (flip 0.5) e1 e2))))
 ...
 (define (bitch-and-moan i)
  (write i)
  (newline)
  (fuck-up))
 
 (define (an-x category words)
  (unless (memq category '(d n a p v coord c nbar np pp vp s r)) (bitch-and-moan 0))
  ...
  (amb
   (list
    (a-member-of
     (remove-if-not
      (lambda (entry)
       (and (eq? (second entry) (force-stochastic-promise (first words)))
	    (eq? (first entry) category)))
      *lexicon*))
    (rest words))
 ...
 (define (head-n nbar)
  (unless (eq? (first nbar) 'nbar) (bitch-and-moan 6))
  (cond ((and (= (length nbar) 4)
	...
	(else (bitch-and-moan 7))))

 (define (head-a? nbar)
  (unless (eq? (first nbar) 'nbar) (bitch-and-moan 8))
  (cond ((and (= (length nbar) 4)
	      (eq? (first (second nbar)) 'nbar)
	      (eq? (first (third nbar)) 'coord)
	      (eq? (first (fourth nbar)) 'nbar))
	...
	(else (bitch-and-moan 9))))

 (define (head-a nbar)
  (unless (eq? (first nbar) 'nbar) (bitch-and-moan 10))
  (cond ((and (= (length nbar) 4)
	      (eq? (first (second nbar)) 'nbar)
	      (eq? (first (third nbar)) 'coord)
	      (eq? (first (fourth nbar)) 'nbar))
	...
	((and (= (length nbar) 3)
	      ...
	      (eq? (first (third nbar)) 'nbar))
	 (head-a (third nbar)))
	((eq? (first (second nbar)) 'n) (bitch-and-moan 11))
	((eq? (first (second nbar)) 'a) (second (second nbar)))
	(else (bitch-and-moan 12))))
	 ...
	 ((vp)
	  (let ((v (second (second parse)))
	   ...
	   (case (- (length parse) 2)
	    ...
	    (else (bitch-and-moan 18)))))
	 ((s) ((loop (third parse)) (loop (second parse))))
	 ((r) (loop (third parse)))
	 (else
	  (write parse) (newline)	;debugging
	  (bitch-and-moan 19)))))))
    ...
    (define (lookup t)
     (let ((binding
	    (find-if (lambda (binding) (eq? t (binding-variable binding)))
		     bindings)))
      (unless binding (bitch-and-moan 23))
     ...
     ((forward-diagonal-square)
      (and (board-square? (lookup (second lf)))
	   ...
	   (= (board-square-row (lookup (second lf)))
		  ...
		  ((x) 1)
		  ((o) -1)
		  (else (bitch-and-moan 32)))
		 (board-square-row (lookup (fourth lf)))))))
     ;; 4
     ((moving) (fail))
     (else (bitch-and-moan 34))))
   (define (function lf bindings state)
    (define (lookup t)
     (let ((binding
	    (find-if (lambda (binding) (eq? t (binding-variable binding)))
		     bindings)))
      (unless binding (bitch-and-moan 35))
      (binding-thing binding)))
    (case (first lf)
     ((then)
      (map-reduce append
		   ...
		   (case (cache-square-player (lookup (fourth lf)))
		    ...
		    ((o)
		     (make-state (state-board state)
				 ...
				 (cache-replace state (lookup (fourth lf)) #f)
				 (state-player state)))
		    (else (bitch-and-moan 58))))
		  (else (bitch-and-moan 59)))))
			...
			(case (cache-square-player (lookup (fifth lf)))
			 ...
			 ((o) (make-state (state-board state)
					  ...
					  (cache-replace
					   ...
					   (piece-player (lookup (third lf))))
					  (state-player state)))
			 (else (bitch-and-moan 60))))
		       (else (bitch-and-moan 61)))))
	  '()))
     (else (bitch-and-moan 62))))
   (define (assertion lf bindings state)
    (define (lookup t)
     (let ((binding
	    (find-if (lambda (binding) (eq? t (binding-variable binding)))
		     bindings)))
      (unless binding (bitch-and-moan 63))
      (binding-thing binding)))
     ...
     ((forward-diagonal-square)
      (assert (and (board-square? (lookup (second lf)))
		   ...
		   (= (board-square-row (lookup (second lf)))
			  ...
			  ((x) 1)
			  ((o) -1)
			  (else (bitch-and-moan 74)))
			 (board-square-row (lookup (fourth lf))))))))
     ;; 4
     ((moving) (fail))
     (else (bitch-and-moan 76))))
   ((case kind
     ((predicate) predicate)
     ((function) function)
     ((assertion) assertion)
     (else (bitch-and-moan 77)))
